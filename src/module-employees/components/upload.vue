<template>
  <div class=''>
    <div v-if="positiveText===5">
      <el-upload action="/api/system/upfile" :data="fullUpType" multiple :on-exceed='handleFile' :limit='limits'
                 :class="{disabled:uploadDisabled}" :headers="myheader" :on-success="handleFileSuccess"
    ...
    <div v-else>
      <el-upload action="/api/system/upfile" :data="fullUpType" multiple :on-exceed='handleFileP' :limit='limit'
                 :class="{disabled:uploadDisabled}" :headers="myheader" :on-success="handleFileSuccess"
                 :file-list="fileList" :on-remove="handleRemove">
        <el-button size="small" type="primary">点击上传</el-button>
