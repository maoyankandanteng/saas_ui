  ...
  <div class="dashboard-container">
    <div class="app-container">
      <el-card>
        ...
        <div class="fr">
          ...
          <router-link :to="{'path':'/employees/report/'+ totals.month}"
                       class="el-button el-button--primary el-button--mini">{{totals.month}}月份报表
          </router-link>
        </div>
      </el-card>
      <el-card shadow="never" v-loading="loading" class="boxMar" :style="{minHeight:boxHeight}">
        ...
        <el-alert v-if="barSearch.alertText !== ''" :title="barSearch.alertText" type="info" class="alert"
        ...
        <!-- 数据表格 / -->
        <!-- 新增员工弹层 -->
        <component v-bind:is="employeesAdd" ref="addUser" :formData.sync='requestParameters' :text='text'
                   :pageTitle='pageTitle' :formBase='formData' :baseData='baseData' :ruleInline='ruleInline'
                   @clearFormDate='clearFormDate' v-on:newDataes="handleLoadDataList"
                   v-on:handleCloseModal="handleCloseModal" @doQuery='doQuery'>
        </component>
        <!-- 转正 -->
        <component v-bind:is="employeesPositive" ref="positive" :objInfoDataes='objInfoDataes'
                   :formData.sync='requestParameters' :text='text' :pageTitle='pageTitle' :formBase='formData'
                   :ruleInline='ruleInline' @clearFormDate='clearFormDate' v-on:newDataes="handleLoadDataList"
                   v-on:handleCloseModal="handleCloseModal">
        </component>

        <!-- 设置 -->
        <component v-bind:is="employeesSet" ref="set" :objInfoDataes='objInfoDataes' :formData.sync='requestParameters'
                   :setData='setData' @clearFormDate='clearFormDate' v-on:newDataes="handleLoadDataList"
                   v-on:handleCloseModal="handleCloseModal">
        </component>
      </el-card>
    </div>
